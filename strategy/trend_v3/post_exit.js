...
function updatePostExitAnalytics(vouchers, closedCandles) {
  ...
  for (const voucher of rows) {
    ...
    const riskPerUnit = Number(voucher.plannedRiskAmount) > 0 && Number(voucher.positionSize) > 0
      ? Number(voucher.plannedRiskAmount) / Number(voucher.positionSize)
      : Number(voucher.plannedR || 0);
